<?php
  class Game {
    ...
    public function __construct($a=array(""), $b=array(""), $c="", $d="", $e=array("",""), $f=array("",""), $g=false, $h=false) {
      ...
      if( $c != "") {
        $this->img0 = '<img title="'.$c.'" src="'.'/FIFA-Generator/logos/16/'.strtolower(preg_replace("/[^a-zA-Z0-9\-]+/", "", html_entity_decode(str_replace(" ", "-", $c), ENT_QUOTES))).'.png'.'">';
      }

      if( $d != "") {
        $this->img1 = '<img title="'.$d.'" src="'.'/FIFA-Generator/logos/16/'.strtolower(preg_replace("/[^a-zA-Z0-9\-]+/", "", html_entity_decode(str_replace(" ", "-", $d), ENT_QUOTES))).'.png'.'">';
      }
